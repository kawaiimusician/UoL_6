...
sys.path.append("/home/rheyman/UoL_6/AWD/topic2/bioweb")
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'bioweb.settings')
django.setup()

from genedata.models import *
data_file = '/home/rheyman/UoL_6/AWD/topic2/example_data_to_load_2.csv'
...
with open(data_file) as csv_file:
  ...
  for row in csv_reader:
    ...
    for pair in product_pairs:
      ...
      products[row[0]][key] = value
    for pair in attribute_pairs:
      if not pair:
        continue
      key, value = pair.split(":", 1)
      attributes[row[0]][key] = value
    ec.add(row[8])
...
for gene_id, data in genes.items():
  row = Gene.objects.create(gene_id=gene_id, entity = data[0], start=data[1], end=data[2], sense=data[3], start_codon=data[4], sequencing = sequencing_rows['Sanger'], ec=ec_rows[data[5]])
  row.save()
